...
- **Работа с данными**: парсинг вакансий с hh.ru, Telegram-каналов и других источников; очистка, нормализация, mаппинг навыков  
...
На данном этапе мы загрузили 53 000 вакансии с hh.ru и провели первичный анализ 
...
На данном этапе экспериментируем с DBSCAN , создав эмбединги навыков по группам: русские , английские и смешанные.
...
Возвращаем проверку смыслового соответсвия синонима канону , а так же проверку на IT навык. 
...
Делаем первичный EDA . Производим добор пропущенных значений (опыт, регион, навыки и т.д.) через разработанные промпты.  
[Смотреть ноутбук](LLM_Mistral_API_labeling.ipynb)

Обучаем модель регрессии, получаем предсказания з/п и первые метрики для дальнейшей оценки изменения качества.  
...
Проводим подбор входных параметров, обучаем Random Forest, получаем результ с улучшенной метрикой и готовим модель на тестовый сервер для демонстрации промежуточных результатов.  [Смотреть ноутбук](stage_3_ml.ipynb)

#### <a id ='stage_4'> 3.7 Разработка мапинга position to skill (профессии к навыку) для альтернативных источников вакансий.

Для мапинга вакансий , которые будем собирать через альтернативные hh.ru источники , проработали вариант создания новых position (профессий).   
...
#### <a id ='tg_LLM'> 3.10 Разметка данных из телеграмм каналов и других сайтов. Получение position через LLM.
...
Задачей этого этапа стала разработка пайплайна , который не только определяет каноны навыков по каждой профессии, но и ищет синонимы по всей профессии.
...
- Пилотные рекомендации по резюме показали, что корректировка навыков и форматирования даёт лучшие отклики со стороны HR
