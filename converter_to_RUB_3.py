...
def convert_to_rub(row):
    ...
    if not pd.isna(row['salary_from_comb']):
        row['salary_from_comb'] = round(row['salary_from_comb'] * rate)
    if not pd.isna(row['salary_to_comb']):
        row['salary_to_comb'] = round(row['salary_to_comb'] * rate)
